...
from src.catsample import sample_categorical
...
class Absorbing(Graph):
    ...
    def transp_transition(self, i, sigma):
        ...
        edge = (-sigma).exp() * F.one_hot(i, num_classes=self.dim) # 从每个状态 i 出发的转移概率
        edge += torch.where(
            i == self.dim - 1,
    ...
    def sample_transition(self, i, sigma):
        ...
        return i_pert # (batch_size, length)
    ...
    def score_entropy(self, score, sigma, x, x0):
        rel_ind = x == self.dim - 1 # (batch_size, length)
        esigm1 = torch.where(
            sigma < 0.5,
            torch.expm1(sigma),
            torch.exp(sigma) - 1
        ...
        ratio = 1 / esigm1.expand_as(x)[rel_ind] # (8475) p(y)/p(x) = 1 / (e^sigma - 1)
